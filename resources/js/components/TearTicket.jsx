...
export default function TearTicket({
  ...
  className = '',
  // Extension propia (no del componente original): un video que
  // reemplaza a la imagen mientras el mouse esta encima del cuerpo del
  // boleto, igual al hover que ya tenia la tarjeta antes de este
  ...
  // Extension propia: hover sobre el arte del cuerpo reproduce el video
  // en vez de la imagen estatica (el componente original no tiene esto).
  const onArtEnter = () => {
    if (!video) return;
        ...
        <motion.div className="tear-ticket__plane" style={{ transform: plane }}>
          <div ref={bodyRef} className="tear-ticket__piece">
            ...
            <div className="tear-ticket__paper" style={{ clipPath: `path('${geo.body}')` }}>
              {image ? (
                <div className="tear-ticket__art" onMouseEnter={onArtEnter} onMouseLeave={onArtLeave}>
                  <motion.img
                    ...
                    draggable={false}
                    style={{
                      ...(reduce ? undefined : { transform: art }),
