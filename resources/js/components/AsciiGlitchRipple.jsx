...
/**
 * `children` debe ser un string simple (se hace `.split('')` y se pisa
 * `el.textContent` a mano durante la animacion) — no JSX anidado.
 * `as`: elemento a renderizar (default 'a' en la fuente original; en
 * GoLeftShowcase.jsx se usa 'span', ver Field/gls-field-overlay).
 */
export function AsciiGlitchRipple({
  ...
  return (
    <Component
      ref={elRef}
      className={`cursor-pointer select-none relative inline-block transition-colors duration-200${className ? ` ${className}` : ''}`}
      {...props}
    />
  );
}
